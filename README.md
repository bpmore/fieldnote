...
**Recolor any theme â€” it won't let you break accessibility.** The palette
...
**Zero JavaScript, genuinely.** Public pages ship markup and one 4â€“8 KB
...
**A single password â€” plus passkeys and real two-factor.** One password
...
sign in with that â€” phishing-proof, with the password always available as
a fallback. Changing the password logs out every other device.

**Flat files you can grep â€” and a real escape hatch.** Posts live in a
...
zip back â€” or any archive of `.md` files with Jekyll/Hugo/Bear-style
frontmatter â€” with a dry-run screen first and a no-duplicates guarantee
...
1. **DNS** â€” an A/AAAA record (or CNAME) for your domain pointing at the
   server.
2. **Web server** â€” serve the domain with its document root on `public/`.
   Apache works out of the box (`.htaccess` ships in `public/`); nginx
   needs `try_files $uri /index.php?$query_string;` in the site block.
3. **HTTPS** â€” issue a certificate at the server or proxy (Let's Encrypt,
   Cloudflare, â€¦). Fieldnote follows whatever the server terminates.
4. **Settings â†’ Blog domain** â€” set it to `https://yourdomain.com`. This
...
5. **Behind Cloudflare or a reverse proxy?** Fill in Settings â†’ trusted
...
Settings â†’ "Two-factor login" â†’ Set up. Scan the QR code with any TOTP
authenticator app (1Password, Google Authenticator, Authy, Apple Passwords, â€¦),
...
Settings â†’ "Passkeys" â†’ Add a passkey. Sign in afterwards with Touch ID /
Face ID / your security key â€” no password typed. Notes:
...
- They're bound to the site's domain; changing the domain orphans them
  (password login is unaffected â€” just re-register).
...
the first visitor to arrive after that moment publishes it â€” with the
...
`/feed.json`, a sitemap at `/sitemap.xml` (referenced from `/robots.txt`) â€”
...
All assets are self-hosted: no CDNs, no webfonts. Public pages ship a single 4â€“8 KB theme stylesheet and no JavaScript, with automatic dark mode.
...
| `bistro` | CafÃ© menu: dotted leaders, chalkboard dark mode |
...
benlk) â€” same spirit, all-new code.
